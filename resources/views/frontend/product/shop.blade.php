		...
		<div class="row">
						...
						<div class="col-md-4 text-center">
							<div class="product-entry">
								<div class="product-img" style="background-image: url(../uploads/{{$product->image}});">
									<div class="cart">
										<p>
											<span class="addtocart"><a href="/vietproshop/gio-hang/them-gio-hang?id_product={{$product->id}}"><i
														class="icon-shopping-cart"></i></a></span>
											<span><a href="{{route('shop.detail',['slug'=>$product->slug])}}"><i class="icon-eye"></i></a></span>
										</p>
									</div>
								</div>
								<div class="desc">
									<h3><a href="{{route('shop.detail',['slug'=>$product->slug])}}">{{ $product->name }}</a></h3>
			...
			<div class="col-md-3 col-md-pull-9">
				<div class="sidebar">
													...
													<ul>
														@foreach ($categories as $children)
															@if ($children->parent == $item->id)
																<li><a href="{{ route('cate.index',['cate_slug'=>$children->slug]) }}">{{ $children->name }}</a></li>
															@endif
														@endforeach
													</ul>
												</div>
					...
					<div class="side">
						<h2>Khoảng giá</h2>
						<form action="{{ route('shop.filter') }}" method="get" class="colorlib-form-2">
							<div class="row">
								<div class="col-md-12">
